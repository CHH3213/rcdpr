...
def env_generate():
    '''数据准备'''
    FileName = 'save/data/value_data_test_2021-11-10-19:07:14.mat'
...
def random_generate(number, is_test=False):
    ...
    # x_uav = [x_uav_1_x,x_uav_1_y,x_uav_2_x,x_uav_2_y,x_uav_3_x,x_uav_3_y,x_uav_4_x,x_uav_4_y]
    # x_uav_ = []
    # for i in range(8):
    #     uav_temp = x_uav[i]
    ...
    # param_var = [param_var_x,param_var_y,param_var_z,param_var_euler,copy.deepcopy(param_var_euler),copy.deepcopy(param_var_euler)]
    # param_var_ = []
    # for i in range(6):
    #     temp = param_var[i]
    ...
    if not is_test:
        # if not True:
        if os.path.exists('data/-5_5_-2_2_96000.mat'):
            train_data = sio.loadmat('data/-5_5_-2_2_96000.mat')
            train_y = train_data['train_y']
            train_x = train_data['train_x']
        else:
            ...
            print(np.shape(train_y))
            print(np.shape(train_x))
            sio.savemat('save/data' + '/fit_version6_v2_data.mat',
                        {'train_x': train_x, 'train_y': train_y})
        print('处理完毕')
        print('train_x shape ', np.shape(train_x))
        print('train_y shape', np.shape(train_y))
    else:
        # x_uav = np.array([[2.5834e+00,  1.5309e+00, -9.0357e-02, -2.8193e+00,  2.4128e+00,1.5324e+00, -5.5237e-02,  8.7681e-02]])
        # param_var = np.array([[-6.4792e-04,  2.9167e-03,3.9965e+00, -9.7495e-03,  1.9739e-02, -1.0223e-02]])
...
if __name__ == '__main__':
    ...
    if train == True:
        ...
        d_step_size = 50000
        scheduler = StepLR(optimizer, step_size=d_step_size, gamma=0.95)
        # loss funaction
